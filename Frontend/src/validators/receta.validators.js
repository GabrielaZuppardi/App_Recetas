...
export const crearRecetaSchema = Joi.object({
  ...
  descripcion: Joi.string().trim().min(10).max(300).required().messages({
  ...
  ingredientes: Joi.string().trim().min(5).required().messages({
  ...
  pasos: Joi.string().trim().min(10).max(1000).required().messages({
  ...
  tiempoPreparacion: Joi.number().integer().min(1).required().messages({
  ...
  porciones: Joi.number().integer().min(1).required().messages({
  ...
  imagenUrl: Joi.string().trim().uri().optional().allow('').messages({
...
export const editarRecetaFormSchema = Joi.object({
  ...
  ingredientesTexto: Joi.string().trim().min(5).required().messages({
    'string.empty': 'Los ingredientes no pueden estar vacíos',
    'string.min': 'Los ingredientes deben tener al menos 5 caracteres',
    'any.required': 'Los ingredientes son obligatorios'
  }),

  pasosTexto: Joi.string().trim().min(10).max(1000).required().messages({
    'string.empty': 'Los pasos no pueden estar vacíos',
    'string.min': 'Los pasos deben tener al menos 10 caracteres',
    'string.max': 'Los pasos no pueden superar los 1000 caracteres',
    'any.required': 'Los pasos son obligatorios'

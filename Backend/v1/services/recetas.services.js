...
export const crearRecetaService = async (receta, usuarioId) => {
  ...
  const recetaExistente = await Receta.findOne({
    titulo: new RegExp(`^${titulo.trim()}$`, "i"),
    usuario: usuarioId
  });
...
export const generarDescripcionRecetaService = async (receta) => {
  const prompt = `
  Generá una descripción para esta receta:
  Reglas:
  Máximo 500 caracteres.
  No uses listas ni viñetas.
  Escribí un único párrafo.
...
export const sugerirDescripcionParaRecetaService = async (id, usuarioLogueado) => {
  if (!isValidObjectId(id)) {
    const error = new Error("El id no es válido");
    error.status = 400;
    throw error;
